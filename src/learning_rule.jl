...
function update_weights!( rule::ForceRule, net::SampleOutputNetwork, task::AbstractTask )
  #=BLAS.gemv!('N', 1.0, rule.P, net.neuron_out, 0.0, rule.k) # update the helper var k=#
  d = calc_k_sample!(rule.k, rule.P, net.neuron_out, net.output_neurons)
  c  = 1/(1 + d)   # helper var
...
function calc_k_sample!( k::Vector{Float64}, P::Matrix{Float64}, neuron_out::Vector{Float64}, output_neurons::Vector{Int} )
  d = 0.
