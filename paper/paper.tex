...
properties of a NN are important for its learning success under a variant
of the FORCE algorithm \citep[Architecture A]{FORCE}, and how to choose
their values for an optimal result.
...
Instead of fine tuning single internal weights, we were thus concerned with the
general rules of the network's properties. And we wanted our networks to
be random to a certain degree, which seems biologically plausible; but we
also wanted to allow for structural specialization:
How densely are the synapses distributed? How strong shall the feedback
signal be, and how strong should the synapses fire on average?
...
Euler integration with a time steps $dt = 0.01$, which was
...
fixed intervals, we used an adaptive mechanism to
determine how often learning should occur. 
...
occupation probability $p$ (for random Erdös-Renyi networks), and the
neighborhood range $k$ (for ring topologies). We furthermore introduced ratio parameters $r$
...
r')\in\{0, 1\}$ that decides whether the NN $\mathcal{N}$
...
% TODO this number is no longer right: we have low, high and full
...
\begin{description}[itemsep=1ex, leftmargin=0pt]
 ...
 \item[Selection] We choose $p=50\%$ of the most successful members of $G_t$ as survivors.
