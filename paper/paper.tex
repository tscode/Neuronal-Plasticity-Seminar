...
- $N=100$ fixed, since greater $N$ always increased quality and computational cost
increased quadraticaly to $N$
- Mutation rate

\section{Results}
\subsection{Frequency Dependent Optimizations}
- short description: get feeling for frequency range (by using unoptimized
parameters from the force paper), make optimizations with 3 different
challenges: 1) challenges that cover the frequency area completely,
challenges that cover low frequencies, and challenges that cover high
frequencies, then pick genotypes with high fitness.
...
- Problems
    * Many magic parameters scattered in the model that were not examined exhaustively
