...
evaluation of $E(F_C)$, but since $G(\lambda)$ depends noncontinuously on $\lambda$, it can only be applied to the dependence 
...
\begin{description}
 \item[Evaluation] The fitness $f_i = F_C(g_i)$ of each individual is mesasured using at least 20 samples (and adaptively more if the $f_i$ are spaced closely, to at msot 100).
