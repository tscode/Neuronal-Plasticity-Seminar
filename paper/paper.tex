...
\begin{abstract}
    ...
    networks, like the local synapse densities, affect their
    learning success for various tasks. To do so, we look at a rate based
    model for neural networks and apply a learning algorithm called FORCE to let the network learn 
		to reproduce either low or high frequent sinusoidals. A sophisticated matching algorithm
    allows us to quantify the learning success of a network so that the
    fitness of structural characteristics can be evaluated. We then use evolutionary
    optimization methods on these parameters, and found that for high frequencies, the
		$\feedback$ parameter can be increased  to improve learning results, at the cost of strongly 
		decreasing the ability to memorize low frequencies. Alternatively, using ring topology instead
		of random connections has a similar effect, but still retains the ability to learn lower frequencies.
\end{abstract}
...
formula, as is done in \citep{FORCE} and \citep{RM}.}
\begin{equation}
...
fixed intervals, we used an adaptive mechanism to determine how often
...
for learning sucess, we introduce \emph{network genotypes}.
...
$\omega\Ix{fb}$. Optimized parameters of type (1) are the
occupation probability $\ERprob$ for sparse random topologies (Erdös-Renyi
topology), the neighborhood range $\RINGk$ for ring topologies, and a ratio
parameter $\ERtoRING$ used to interpolate between Erdös-Renyi and ring
topologies. Optimized parameters of type (2) are the feedback strength
$\feedback\propto \omega\Ix{fb}$ and the gain value
$\gain\propto\omega\Ix{rec}$.


\paragraph{Fitness} The next step was to define the \emph{fitness} of
...
$\nu\in[0.3, 3]$ as frequency range for the challenge $C\ix{high}$, producing
high frequency waves, and $\nu\in[0.004, 0.04]$ for $C\ix{low}$, producing low
frequency waves. 
Optimizing the fitness functions $F_{C\ix{high}}$ or $F_{C\ix{low}}$ thus
results in genotypes with the desired frequency properties.
...
$\mathcal{N}$ with readout $z$ for the task $z'$ was (by empirical means)
taken to be 
\begin{equation}
...
even one learning process is considerably high, these methods were not
feasible.
...
Therefore, we optimized $F_C$ using a genetic algorithm, which is inherently
...
\begin{description}[itemsep=1ex, leftmargin=0pt]
 ...
 \item[Selection] We choose $50\%$ of the most successful members of $G_t$ as survivors.
...
\item[Reproduction] We then replace the $(1-p)|G_t|$ elements that did not make
    it into the next generation by either \emph{mutating} or \emph{recombining} the 
...
But how well was the fitness actually improved during the optimization?
...
high frequencies without increasing the feedback, we performed an
optimization in which the network topologies were an interpolation between
...
probability for Erdös-Renyi topology $\ERprob$, the ring connection length $\RINGk$
and the interpolation value $\ERtoRING$ between Erdös-Renyi and ring as
free parameters.
...
first few generations is extremely low. The development of the ration
parameter, however, unambiguously shows that the selection process prefers
...
The fitness curve of $G\ix{high,ring}$ in figure \ref{fig:freqrange}
...
frequencies here, the code developed during our project provides
...
final genotypes varied quite a bit in their fitness, we did not observe
...
parameters showed less influence on the learning results than we initially
